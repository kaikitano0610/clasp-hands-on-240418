...
const chatGPT_suggest_tag = (text: string, replyToken: string, userId: string): void => {
  ...
  test("ChatGPT の API を呼び出す", async () => {
    ...
    newRow[columnIndexMap.tags] = response.data.choices[0].message?.content;
    ...
    const messages = [
      {
        type: 'text',
        text: 'chatGPTによって提案されたタグ「'+response.data.choices[0].message?.content+'」タスク「'+ lastTaskContent +'」で登録しました',
      },
    ]
